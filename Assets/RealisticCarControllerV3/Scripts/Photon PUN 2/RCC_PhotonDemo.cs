...
public class RCC_PhotonDemo : Photon.Pun.MonoBehaviourPunCallbacks {
    ...
    public void Spawn(string carName) {
        ...
        newVehicle = PhotonNetwork.Instantiate(carName, lastKnownPos + (Vector3.up), lastKnownRot, 0).GetComponent<RCC_CarControllerV3>();
    ...
    public override void OnJoinedRoom() {
        ...
        if (menu)
            menu.SetActive(true);
        GameManager.Instance.player = PhotonNetwork.Instantiate("Player",Vector3.zero, Quaternion.identity);
        UIInventoryManager.Instance.InitializeMainMenu(GameManager.Instance.player.GetComponent<Inventory>());
    }
